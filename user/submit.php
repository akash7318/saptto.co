...
$direct= strtolower($_POST['sponsor']);
...
function getpos(){
    ...
    $sql  = "SELECT id,sponsor,position FROM `join` order by id DESC limit 1";
    ...
    if($lpos<3){
        ...
        $sql  = "SELECT id FROM `join` where username = '$lsponsor'";
        $result = mysqli_query($s_dbid,$sql);
        list($lid) = mysqli_fetch_row($result);
        
        $lid++;
        
        $sql  = "SELECT username FROM `join` where id = '$lid'";
        $result = mysqli_query($s_dbid,$sql);
...
function getNodeInsertPostionByParentId($parentId){
        ...
        foreach($parentId as $parent){  
                ...
                $qry = "select username,position from `join` where sponsor ='".$parent."' order by position";
...
//check for validations
    ...
    $sql  = "SELECT * FROM `join` WHERE `phone` = '$phone'";
    $result = mysqli_query($s_dbid,$sql);
    $nrows=0;
    $nrows = mysqli_num_rows($result);    
    if ((int)$nrows>0){
    ...
    $sql  = "SELECT * FROM `join` WHERE `username` = '$direct'";
    $result = mysqli_query($s_dbid,$sql);
    $nrows=0;
    $nrows = mysqli_num_rows($result);		
    if ((int)$nrows<=0){
...
//echo $flag."test";
		...
		$msg="";
			...
			$sqlc  = "SELECT username FROM `join` order by id desc";
			...
			$sql = "INSERT INTO `join` VALUES (NULL, '$name', '$phone', '$email', '$direct', '$username', '$password', NULL, '$direct',  NULL, 'pending', '$jdate', '$tpassword',  '$title', '$country', '$state', '$city', '$address','$pincode');";
			$result = mysqli_query($s_dbid,$sql);
            ...
            $sql_bank = "INSERT INTO `bank` (`id`, `bank_name`, `branch_name`, `account_no`, `account_type`, `aname`, `jid`, `ifsc`, `pancard`, `paytm`, `phonepe`, `googlepay`, `upi`, `panimage`, `afront`, `aback`, `idnumber`, `idtype`, `panstatus`, `idstatus`, `bank_status`) VALUES (NULL, NULL, NULL, NULL, NULL, NULL, '$jid', NULL, '$pan', NULL, NULL, NULL, NULL, NULL, NULL, NULL, '$aadhaar', NULL, NULL, NULL,NULL);";
...
$message = "Dear ".$name." You have Successfully Registered. Your User ID is ".$username." Password is ".urlencode($password)." Thank You for joining with us. Spatto Travel www.spatto.co";
